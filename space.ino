...
void buttonsSpace() {
  ...
  if (left == BTN and fighterX > -4) {
    fighterX -= 1;
  }
  if (right == BTN and fighterX < 124) {
    fighterX += 1;
  }
  if (shoot == BTN) {
    fighterShoot();
  ...
  if (rocket == BTN and gamemode == 1 and rockets > 0 and (millis() - laserTimer) > 2000) {
    laserX = fighterX + 4;
    laserTime = 0;
...
void playSpace() {
  ...
  int shoot = !BTN;
  int rocket = !BTN;
  while (left == !BTN and right == !BTN and shoot == !BTN and rocket == !BTN) {
    left = digitalRead(KEYLS);
    right = digitalRead(KEYRS);
    shoot = digitalRead(KEYRC);
    rocket = digitalRead(KEYLC);
