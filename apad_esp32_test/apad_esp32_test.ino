...
void gamMenu() {
  ...
  while (enter == 0) {
    ...
    switch (mapnum) {
      ...
      case 2:
        ...
        break;
      case 3:
        display.drawBitmap(47, 23, thermometer_logo, 34, 34, 1);
        break;
      case 4:
        display.drawBitmap(47, 23, phisics_logo, 34, 34, 1);
        break;
      case 5:
        display.drawBitmap(47, 23, piano_logo, 34, 34, 1);
        break;
      case 6:
      ...
      case 8:
        ...
        break;
      case 9:
        display.drawBitmap(47, 23, lambda_logo, 34, 34, 1);     // Выводим иконку на дисплей
        break;
      case 10:
  ...
  if (enter == 1) {
    ...
    else if (mapnum == 9) {        // Если пользователь выберет 10 приложение
      playMyApp();             // Запустим наше
    }
    else if (mapnum == 10) {
      playBeer();
    }
...
byte korobkaMenu(byte lenght, const char *elements[]) {
  ...
  while (1) {
    ...
    bool down = 1;
    bool enter = 1;
    while (up and down and enter) {
      up = digitalRead(KEYLS);
      down = digitalRead(KEYRC);
      enter = digitalRead(KEYRS);
    }
    if (!up) mapnum--; if (mapnum < 0) mapnum = lenght - 1; delay(100);
    if (!down) mapnum++; if (mapnum > lenght - 1) mapnum = 0; delay(100);
    if (!enter) break;
  }
  delay(200);
  return mapnum;
...
int korobkaInput(int mini, int maxi, int stepi, int def) {
  ...
  while (1) {
    ...
    if (!up) mapnum -= stepi; if (mapnum < mini) mapnum = maxi - 1; delay(100);
    if (!down) mapnum += stepi; if (mapnum > maxi - 1) mapnum = mini; delay(100);
    if (!enter) break;
  }
  delay(200);
  return mapnum;
    ...
    switch (korobkaMenu(4, settings)) {
      ...
      case 3: {
          if (korobkaInput(0, 1000, 10, 500) == ROM_PASSWD) {
            ...
            message("Data:", 400);
            writeInt(addr, korobkaInput(0, 65535, 1, readInt(addr)));
            message("Saved!", 400);
          }
          else {
